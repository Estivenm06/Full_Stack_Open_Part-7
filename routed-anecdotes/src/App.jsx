...
const Anecdote = ({ anecdotes }) => {
  return (
    <div>
      ...
      <div>
        <span>has {anecdotes.votes} votes</span>
        <br />
        <span>for more info see</span>
        <Link to={`${anecdotes.info}`}>{anecdotes.info}</Link>
      </div>
    </div>
  );
};
...
const App = () => {
  ...
  const addNew = (anecdote) => {
    ...
    anecdoteArray.id = Math.round(Math.random() * 10000);
    setAnecdotes(anecdotes.concat(anecdoteArray));
    setNotification(`A new anecdote ${anecdoteArray.content} created!`);
    setTimeout(() => {
